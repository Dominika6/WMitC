...
export const updateClientEmail = async (req, res) => {
    ...
    try {
        ...
        if(!existingClient) return res.status(404).json({ messsage: "Client doesn't exist." });
...
export const updateClientPhone = async (req, res) => {
    ...
    try {
        ...
        if(!existingClient) return res.status(404).json({ messsage: "Client doesn't exist." });
        if(!mongoose.Types.ObjectId.isValid(id)) return res.status(404).send('No client with that id.');
        await Clients.findByIdAndUpdate(id, { phone_number: phone_number }, { new: true });
        res.status(200).json({ result: existingClient});
    } catch ( error) {
        res.status(500).json({message: 'Something went wrong.'});
    }
...
export const updateClientCoordinator = async (req, res) => {
    ...
    try {
        ...
        if(!existingClient) return res.status(404).json({ messsage: "Client doesn't exist." });
        if(!mongoose.Types.ObjectId.isValid(id)) return res.status(404).send('No client with that id.');
        await Clients.findByIdAndUpdate(id, { id_supervisor: id_supervisor }, { new: true });
        res.status(200).json({ result: existingClient});
    } catch ( error) {
        res.status(500).json({message: 'Something went wrong.'});
    }
