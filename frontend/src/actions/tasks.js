import { CREATE_TASK, DELETE, START_LOADING, END_LOADING, FETCH_TASK, FETCH_BY_SEARCH, UPDATE_TASK, COMMENT } from "../constants/actionTypes";
...
export const getTasksBySearch = (email) => async (dispatch) => {
    try {
        dispatch({ type: START_LOADING });
        const { data: { data } } = await api.fetchTasksBySearch(email);
...
export const getTeamTasksBySearch = (email) => async (dispatch) => {
    try {
        dispatch({ type: START_LOADING });
        const { data: { data } } = await api.getTeamTasksBySearch(email);
        dispatch({ type: FETCH_BY_SEARCH, payload: data });
        dispatch({ type: END_LOADING });
    } catch (error) {
        console.log(error);
